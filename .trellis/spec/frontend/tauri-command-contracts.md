...
| `OSC 7` is absent | Track simple user-entered `cd` commands as the eager fallback. On manual locate, the active tab may inspect already-rendered prompt lines as a best-effort snapshot fallback. Basename-only prompts such as `[root@host app]#` are usable only when matched to a known current directory or a nearby explicit `cd /absolute/path` command. Keep showing the default/manual path until a path is recorded. Do not parse arbitrary command output and do not write current-directory probes into the interactive terminal. |
