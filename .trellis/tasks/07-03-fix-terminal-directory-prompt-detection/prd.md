...
- CentOS-style prompts such as `[root@host app]#` may show only a directory
...
- [x] A CentOS-style prompt like `[root@192 app]#` can locate `/opt/app` only
      when it matches a known current directory or a nearby reconstructable
      `cd` command chain in the snapshot.
- [x] A CentOS-style relative chain such as `cd /opt/`, `cd app`, `cd jar`,
      and `cd ../softwares/` can locate `/opt/app/softwares` from the
